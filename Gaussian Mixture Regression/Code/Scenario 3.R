...
c<-d[!(abs(d$Beneficiaries) > 3 | abs(d$Female_Ratio) > 3 | abs(d$Age) > 3 | abs(d$Depen_ratio) > 3 | abs(d$Pens_ratio) > 3 ),]
c$Pens_ratio = NULL
...
m1<- flexmix(c$Claims ~ x1 + x2 +x3 + x5, data = c, k = 3)
m1
